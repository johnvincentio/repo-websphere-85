...
public class AppProps implements Serializable {
	...
	private void setDefaults() {
		...
		db2JavaPath = "C:/Program Files (x86)/IBM/SQLLIB/java";
		db2LibPath = "C:/Program Files (x86)/IBM/SQLLIB/lib";
		mqJmsLibRoot = "${MQ_INSTALL_ROOT}/java/lib";
		mySQLLibPath = "c:/development/MySQL/mysql-connector-java-5.1.30-bin.jar";

		editor = "C:/Program Files (x86)/EditPlus 3/editplus.exe";
		browser = "C:/Program Files (x86)/Google/Chrome/Application/chrome.exe";

		if (version == 85) {
			jaclRootPath = "c:/development";
			workspaceServerPath = "c:\\tmp85";

			workspaceDir = "C:/ws85Eclipse";
			serverDir = "C:/profiles85";

			workspaceStart = "C:\\Applications\\eclipse-jee\\eclipse.exe";
			workspaceStartKey = "";
			workspaceStartValue = "";
	...
	private void process (String file) {
		...
		String tmp;
		if (isValid(tmp = props.getProperty("jacl_root_path"))) jaclRootPath = tmp;
		...
		if (isValid(tmp = props.getProperty("mysql_lib_path"))) mySQLLibPath = tmp;
	...
	public boolean isJaclActive() {
		if (! isValid(jaclRootPath)) return false;
		...
		if (! isValid(mySQLLibPath)) return false;
		return true;
	}
	...
	public String toString() {
		return "("+getVersion()+","+getJaclRootPath()+","+getWorkspaceServerPath()+","+getInstance()+","+
			getBindingLocation()+","+getDb2JavaPath()+","+getDb2LibPath()+","+
			getDb2UniversalDriverLibPath()+","+getMQJmsLibRoot()+","+getSqlserverLibPath()+","+getMySQLLibPath()+","+
...
/*
	private String getClasspathRootPath() {
		...
		for (int num = 0; num < paths.length; num++) {
			...
			if (! paths[num].contains("RadTool")) continue;
			int end = paths[num].lastIndexOf("\\RadTool");
			if (end < 1) continue;
