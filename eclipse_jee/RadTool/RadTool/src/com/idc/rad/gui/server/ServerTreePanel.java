...
public class ServerTreePanel extends JPanel implements ActionListener {
	...
	private JMenuItem[] m_menuItemMainJacl;
...
*/
		...
		if (m_app.isJaclActive()) {
			...
			ArrayList<String> jaclList = m_app.getAppJacl().getJaclList();
			if (! jaclList.isEmpty()) {
				m_menuItemMainJacl = new JMenuItem[jaclList.size()];
				for (int i = 0; i < jaclList.size(); i++) {
					m_menuItemMainJacl[i] = new JMenuItem (jaclList.get(i));
					m_menuItemMainJacl[i] .addActionListener (this);
					m_menuJacl.add (m_menuItemMainJacl[i]);				
				}
			}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() instanceof JMenuItem) {
			...
			if (source == m_menuItemStart) {
				...
				if (strNode != null) m_app.doServersJacl ("jacl_irac", "irac.jacl", null, null, strNode);
			}
			else if (source == m_menuItemHercJacl) {
				if (strNode != null) m_app.doServersJacl ("jacl_herc", "herc.jacl", null, null, strNode);
			}

			else if (source == m_menuItemHercDeployEtrieve) {
				if (strNode != null) m_app.doServersJacl ("jacl_herc", "hercDeploy.jacl", "EtrieveIt", "EtrieveIt.ear", strNode);
			}
			else if (source == m_menuItemHercDeployAdminTool) {
				if (strNode != null) m_app.doServersJacl ("jacl_herc", "hercDeploy.jacl", "AdminToolEAR", "AdminToolEAR.ear", strNode);
			}
			
			else if (source == m_menuItemHercUndeployEtrieve) {
				if (strNode != null) m_app.doServersJacl ("jacl_herc", "hercUndeploy.jacl", "EtrieveIt", null, strNode);
			}
			else if (source == m_menuItemHercUnDeployAdminTool) {
				if (strNode != null) m_app.doServersJacl ("jacl_herc", "hercUndeploy.jacl", "AdminToolEAR", null, strNode);
			}
			...
			else if (m_app.getAppJacl().isExists (source.getText())) {
				if (strNode != null) m_app.doServersJacl (source.getText(), "main.jacl", null, null, strNode);
			}
		}
	}
	public static String makeName (String s1, String s2) {return s1 + File.separatorChar + s2;}
